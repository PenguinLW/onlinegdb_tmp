...
//	    create_ab_list(file);
        ...
        loc_ab_list = new String[list_ab.length];
        for(int i = 0; i <= list_ab.length-1; i++) {
            loc_ab_list[i] = list_ab[i].split("\t")[2];
        }
        
        do {
            System.out.println("1. Задать адрес для поиска абонента\n0. Выход");
            ch = in.nextInt();
            switch(ch) {
                case 1:
                    int index;
                    ...
                    for(int i = 0; i <= loc_ab_list.length-1; i++) {
                        System.out.println("\t"+(i+1)+"\t"+loc_ab_list[i]);
                    }
                    index = in.nextInt();
                    if(index > 0 && index <= list_ab.length) {
                        rez = "По данному адресу ("+loc_ab_list[index-1]+") проживает абонент:\n\t"+list_ab[index-1];
